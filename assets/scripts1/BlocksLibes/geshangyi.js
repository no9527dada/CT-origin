...
blockType.phaseUseTime = 4 * 60;
blockType.shieldHealth = 0;
blockType.phaseShieldBoost = 0;
blockType.radius = 60 * 8;
blockType.phaseRadiusBoost = 80 * 8;
blockType.itemConsumer = blockType.consumeItem(items.weijing2,1).boost();
blockType.requirements = ItemStack.with(
    Items.copper, 4000,
    Items.lead, 2500,
    items.JHhejin, 50,
    items.jin, 300,
    items.guijingti, 1700,
    items.shimoxi, 600,
    items.weijing2, 210,
    items.weijing3, 80
);
...
lib.setBuildingSimple(blockType, ForceProjector.ForceBuild, {
    ...
    drawShield() {
        var COLOR = new Color.valueOf("f85151");
        var COLOR2 = new Color.valueOf("f85151");
        var x = this.x;
        var y = this.y;
        var hit = this.hit;
        if (!this.broken) {
