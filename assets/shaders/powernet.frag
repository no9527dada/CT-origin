...
void main() {
    ...
    float a_mul = (sin(u_time / 22.0) + 1.0) * 0.1 + 0.35;
    ...
    if (texture2D(u_texture, T).a < 0.9 && maxed.a > 0.59) {
        gl_FragColor = vec4(maxed.rgb, 0.7 * a_mul);
    } else {
        if (color.a > 0.0) {
            if (mod(coords.x / u_dp - coords.y / u_dp + u_time / 24.0, 12.0) < 1.0 || mod(coords.x / u_dp + coords.y / u_dp + u_time / 8.0, 12.0) < 1.0) {
