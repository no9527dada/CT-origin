...
(() => {
    ...
    var blockType = extend(Block, "jiasu8", {
        load() {
    ...
    lib.setBuilding(blockType, (block) => {
        ...
        return new JavaAdapter(Building, {
            ...
            drawLight(){
                Drawf.light( this.x, this.y, 50 * this.smoothEfficiency, baseColor, 0.7 * this.smoothEfficiency);
            },
            drawSelect(){
                var realRange = range;
...
/*                     if (speedBoost < 1) {
                        if (speedBoost <= other.block.canOverdrive) {
                            other.timeScaleDuration = Math.max(other.timeScaleDuration, duration);
                        }
                           other.block.canOverdrive = Math.min(other.block.canOverdrive, speedBoost);
                    } else {
