...
public class CTPlanetDialog extends BaseDialog implements PlanetInterfaceRenderer{
    ...
    public PlanetDialog.Mode mode = look;
    ...
    public CTPlanetDialog(){
        ...
        addListener(new InputListener(){
            ...
            public boolean keyDown(InputEvent event, KeyCode key){
                if(event.targetActor == CTPlanetDialog.this && (key == KeyCode.escape || key == KeyCode.back || key == Core.keybinds.get(Binding.planet_map).key)){
        ...
        addListener(new InputListener(){
            ...
            public boolean scrolled(InputEvent event, float x, float y, float amountX, float amountY){
                if(event.targetActor == CTPlanetDialog.this){
    ...
    void rebuildButtons(){
        ...
        if(Core.graphics.isPortrait()){
            ...
            buttons.button("显示周期",Icon.settings,()->{
                ...
                arcInfo("调整资源输入|输出显示周期为 [orange]" + viewInterval(viewInt));
    ...
    void setup(){
        ...
        new Element(){
            ...
            public void act(float delta){
                if(scene.getDialog() == CTPlanetDialog.this && !scene.hit(input.mouseX(), input.mouseY(), true).isDescendantOf(e -> e instanceof ScrollPane)){
                    scene.setScrollFocus(CTPlanetDialog.this);
                }
    ...
    void displayItems(Table c, float scl, ObjectMap<Item, ExportStat> stats, String name, Cons<Table> builder,Boolean showColor){
        ...
        for(var item : content.items()){
            ...
            if(total != 0){
                ...
                t.add(showColor? colorFormatAmount(total) : UI.formatAmount(total)).color(Color.lightGray).padRight(3);
                if( ++i % rowSet == 0){
                    t.row();
                }
            }
