...
public class CTPausedDialog extends BaseDialog {
    ...
    void rebuild(){
        ...
        if(!mobile){
          ...
          //  cont.image().color(Color.valueOf("69dcee")).fillX().height(10).pad(5);
           ...
           // cont.row();
            cont.button(Core.bundle.format("gonglue"), (() -> {
                if (!Core.app.openURI("https://www.bilibili.com/video/BV12u4y1Q7Ft/")) {
                    Vars.ui.showErrorMessage("@linkfail");
                    Core.app.setClipboardText("https://www.bilibili.com/video/BV12u4y1Q7Ft/");
                }
            })).size(210, 64).visible(() ->
                    state.rules.sector != null && state.rules.sector.preset != null
                            && state.rules.sector.preset.description != null).padTop(60f);
           cont.row();
            ...
            }).disabled(b -> !((steam && net.server()) || !net.active())).colspan(2).width(250f).update(e -> e.setText(net.server() && steam ? "@invitefriends" : "@hostserver"));
