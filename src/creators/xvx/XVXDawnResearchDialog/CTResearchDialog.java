...
private Color 点亮颜色=Color.valueOf("78ffab");
    ...
    private Color 测试颜色=  Color.valueOf("ea60ff");
    public CTResearchDialog(){
        ...
        titleTable.button(b -> {
            ...
            b.add().growX();
            b.label(() -> root.node.localizedName()).color(Color.valueOf("fff578"));//科技树分类名字的颜色：Pal.accent
            b.add().growX();
            b.add().size(iconMed);
        }, () -> {
    ...
    public class View extends Group{
        ...
        public String 显示(Objective o){
            if(o instanceof Objectives.Produce){
                var A = (Objectives.Produce) o;
                return Core.bundle.format("requirement.produce", A.content.emoji() + " " + A.content.localizedName);
            }
            if(o instanceof Objectives.Research){
                var A = (Objectives.Research) o;
